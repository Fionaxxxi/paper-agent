...
2026-08-24 已修复复合问候和身份省略句误入论文检索：`你好，先简单介绍一下你能做什么` 与 `你是？` 原先因 Intent Router 仅接受完全匹配的短问候/`你是谁`，会被错误标为 research，执行两轮论文检索并返回无关论文。现增加受限的能力、身份和自我介绍表达模式，只在 60 字以内且明确询问系统身份或能力时本地短路；`你好，请比较 RAG 和 GraphRAG` 仍进入研究流程。本地返回会确定性清空 Documents、Tools、Retry、Evidence Store、QA Skill、LLM Usage 和 Token，页面固定显示 `local_response / local / 0 evidence / 0 LLM / 0 Token`。明确意图优先采用零 LLM 路由，语义复杂的研究请求继续交给 Research Analyzer；未来只对规则低置信度的边界请求增加受限枚举的轻量语义兜底，避免所有输入额外消耗 Token。真实服务层验证仅运行 Intent Router，总耗时约 0.13 秒。
