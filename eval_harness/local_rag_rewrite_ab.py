...
def run_ab(output_path: Path, dataset: Path = Path("eval_harness/datasets/rag_gold_v1.json"), evaluation_role: str = "development_ab_not_promotion_evidence") -> dict:
    ...
    for before, after in zip(baseline["cases"], candidate["cases"]):
        recall_delta = after["metrics"]["recall_at_5"] - before["metrics"]["recall_at_5"]
        ndcg_delta = after["metrics"]["ndcg_at_5"] - before["metrics"]["ndcg_at_5"]
        cases.append({"id": before["id"], "question": before["question"], "search_query": after["search_query"], "rewrite_matches": after["rewrite_matches"], "baseline_first_rank": before["first_relevant_rank"], "candidate_first_rank": after["first_relevant_rank"], "baseline_recall_at_5": before["metrics"]["recall_at_5"], "candidate_recall_at_5": after["metrics"]["recall_at_5"], "chunk_recall_delta": recall_delta, "chunk_ndcg_delta": round(ndcg_delta, 6), "page_recall_delta": after["page_metrics"]["recall_at_5"] - before["page_metrics"]["recall_at_5"], "outcome": "improved" if ndcg_delta > 0 else "regressed" if ndcg_delta < 0 else "unchanged"})
