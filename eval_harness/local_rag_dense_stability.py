...
def analyze(run_paths: list[Path], output_path: Path) -> dict:
    ...
    report = {"report_version":"1.0","run_count":len(runs),"all_cache_hits":all_cache_hits,"fingerprint_count":len({run["cache"]["fingerprint"] for run in runs}),"quality_equal":quality_equal,"top5_rankings_equal":rankings_equal,"scores_equal":scores_equal,"timing":timing,"decision":{"stability_validated":all_cache_hits and quality_equal and rankings_equal and scores_equal and latency_stable,"latency_stable":latency_stable,"production_default":False,"next_step":"第二个多语言 Dense 模型单变量对照"}}
...
if __name__ == "__main__":
    paths=sorted(Path("outputs/local_rag/stability_runs").glob("run_*.json"));print(json.dumps(analyze(paths,Path("outputs/local_rag/dense_stability.json")),ensure_ascii=False,indent=2))
