...
这套评测用于比较 `arxiv`、`openalex` 和 `multi` 在同一批论文问题上的检索质量、延迟与可靠性。它与离线能力基准分开运行，不进入默认 CI，也不会在缺少凭据时偷偷消耗匿名 OpenAlex 额度。
...
正式选型运行必须满足：OpenAlex Key 已配置、三个配置使用同一数据集版本、失败状态已消除或经人工确认、逐题结果与配置完整保存。
