...
def _run(latency, model="model-v1"):
    ...
    warmup={"query":"academic paper semantic retrieval warmup","count":2,"latency_ms":[20,10],"excluded_from_formal_timing":True}
    return {"config":{"model":model},"warmup":warmup,"cache":{"hit":True,"fingerprint":"same"},"timing":{"model_load_ms":100,"cache_load_ms":5,"index_build_ms":2},"development":split,"holdout":split}
...
def test_dense_stability_requires_three_deterministic_warm_processes(tmp_path):
    ...
    assert report["warmup_protocol_match"] is True
